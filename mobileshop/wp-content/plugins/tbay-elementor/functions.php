...
if( ! function_exists( 'tbay_elementor_widget_init' ) ) {
    function tbay_elementor_widget_init() {
    	$widgets = apply_filters( 'tbay_elementor_register_widgets', array('instagram') );
    	if ( !empty($widgets) ) {
    		foreach ($widgets as $widget) {
...
if( ! function_exists( 'tbay_elementor_scrape_instagram' ) ) {
    function tbay_elementor_scrape_instagram( $username ) {
      ...
      $username = trim( strtolower( $username ) );
        ...
        if ( false === ( $instagram = get_transient( 'insta-a10-' . $transient_prefix . '-' . sanitize_title_with_dashes( $username ) ) ) ) {
            ...
            if ( is_wp_error( $remote ) ) {
                return new WP_Error( 'site_down', esc_html__( 'Unable to communicate with Instagram.', 'tbay-elementor' ) );
            }
            if ( 200 !== wp_remote_retrieve_response_code( $remote ) ) {
                return new WP_Error( 'invalid_response', esc_html__( 'Instagram did not return a 200.', 'tbay-elementor' ) );
            }
            $shards      = explode( 'window._sharedData = ', $remote['body'] );
            $insta_json  = explode( ';</script>', $shards[1] );
            $insta_array = json_decode( $insta_json[0], true );
            if ( ! $insta_array ) {
                return new WP_Error( 'bad_json', esc_html__( 'Instagram has returned invalid data.', 'tbay-elementor' ) );
            }
            if ( isset( $insta_array['entry_data']['ProfilePage'][0]['graphql']['user']['edge_owner_to_timeline_media']['edges'] ) ) {
                ...
                return new WP_Error( 'bad_json_2', esc_html__( 'Instagram has returned invalid data.', 'tbay-elementor' ) );
            }
            if ( ! is_array( $images ) ) {
                return new WP_Error( 'bad_array', esc_html__( 'Instagram has returned invalid data.', 'tbay-elementor' ) );
            }
            $instagram = array();
            foreach ( $images as $image ) {
                ...
                $caption = __( 'Instagram Image', 'tbay-elementor' );
        ...
        if ( ! empty( $instagram ) ) {
            ...
            return new WP_Error( 'no_images', esc_html__( 'Instagram did not return any images.', 'tbay-elementor' ) );
        }
    }
}


if( ! function_exists( 'tbay_elementor_instagram_wpiw_proxy' ) ) {
    function tbay_elementor_instagram_wpiw_proxy() {
        return true;
    }
    add_filter('wpiw_proxy', 'tbay_elementor_instagram_wpiw_proxy');
}


if( ! function_exists( 'tbay_elementor_time_ago' ) ) {
    function tbay_elementor_time_ago($distant_timestamp, $max_units = 3) {
        $i = 0;
        
        $time = time() - $distant_timestamp; // to get the time since that moment
        $tokens = array(
            31536000    => esc_html__('year', 'tbay-elementor'),
            2592000     => esc_html__('month', 'tbay-elementor'),
            604800      => esc_html__('week', 'tbay-elementor'),
            86400       => esc_html__('day', 'tbay-elementor'),
            3600        => esc_html__('hour', 'tbay-elementor'),
            60          => esc_html__('minute', 'tbay-elementor'),
            1           => esc_html__('second', 'tbay-elementor')
        );
        ...
        while ($i < $max_units) {
            foreach ($tokens as $unit => $text) {
                ...
                array_push($responses, $numberOfUnits . ' ' . $text . (($numberOfUnits > 1) ? esc_html__( 's', 'tbay-elementor' ) : ''));
        ...
        if (!empty($responses)) {
            return implode(', ', $responses) . esc_html__( ' ago', 'tbay-elementor' );
        }

        return esc_html__('Just now', 'tbay-elementor');
    }
}

if( ! function_exists( 'tbay_elementor_images_only' ) ) {
    function tbay_elementor_images_only( $media_item ) {
...
if( ! function_exists( 'tbay_calculate_image_srcset' ) ) {
    /**
     ...
     */
    function tbay_calculate_image_srcset( $size_array, $image_src, $image_meta, $attachment_id = 0 ) {
        /**
